...
function VoegDossierToe( { onClassSelected , uri_class} ) {
  ...
  return (
    <div>
      <form onSubmit={e => {
        e.preventDefault();
        voegDossierToe({ variables: { label: input.value, uri: (uri_class + "#" + camelCase(input.value)), prefLabel: input.value } });
        ...
        <label for="dossier">Kies het dossier type:</label>
        <select name="dossier" onChange={onClassSelected} required>
        <option value = "" disabled selected> kies het type dossier</option>
          {data.classes.map(
            klasse => (
              <option
                key={klasse.uri}
